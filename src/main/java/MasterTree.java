...
class MasterTree {
     ...
     */
    public synchronized void updateVotes(String pollId, String userId) {
        try {
            PreparedStatement getAnswerRow = connection.prepareStatement("SELECT * FROM ? WHERE 'user_id'=");
            getAnswerRow.setString(1, pollId + "_answers");
            PreparedStatement answerRow = connection.prepareStatement(getAnswerRow.toString().replace("'","\"") + "?");
            ...
            ResultSet questions = connection.createStatement().executeQuery(getQuestions.toString().replace("'", "\""));
            ...
            while (questions.next()) {
                ...
                PreparedStatement updateStatement = connection.prepareStatement("UPDATE ? SET ? = ? + 1 WHERE 'statement_id'=");
                updateStatement.setString(1, pollId);
                updateStatement.setString(2, yesOrNo);
                updateStatement.setString(3, yesOrNo);

                updateStatement = connection.prepareStatement(updateStatement.toString().replace("'","\"") +"?");
    ...
    public synchronized void updateScores(String pollId) {
        try {
            ...
            Argument root = new Argument(true /* Fake */, rootResult.getString("statement"), true /* Fake */);
    ...
    private void setArgumentChildren(Argument parent, String pollId) {
        try {
            PreparedStatement findChildren = connection.prepareStatement("SELECT * FROM ? WHERE 'parent_id'=");
            findChildren.setString(1, pollId);
            findChildren = connection.prepareStatement(findChildren.toString().replace("'", "\"") + "?");
